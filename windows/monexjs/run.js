...
var routes = require(drive + '/monexjs/routes/index');
...
app.set('views', path.join(__dirname, drive + '/monexjs/admin/views'));
...
app.use(express.static(path.join(__dirname, drive + '/monexjs/admin')));
app.use("/node_modules", express.static(__dirname + drive + "/monexjs/admin/node_modules"));


app.use(express.static(drive + '/monexjs/admin'));
app.use('/', routes);
...
app.use(function(req, res, next){
  res.render(drive + 'monexjs/404.html', { status: 404, url: req.url });
});

app.use(function (err, req, res, next) {
  res.render(drive + 'monexjs/505.html', { status: 500, url: req.url });
});
...
port = 9090;
    ...
    fs.appendFileSync('monexjs/logs/node.mnex', `port: ${port} was occupied, try port: ${_port}`);
    }
});
